    ...
    def get_true_pred(self):
        ...
        def prepare(pred):
            for target in self.targets:
                ...
                pred[key] = pred[key].detach().cpu().numpy()
            return pred

        train, val = self.train_valid_data[0]
        train_batch = Batch.from_data_list(train.dataset + val.dataset).to(self.device)
        test_batch = Batch.from_data_list(self.test_data.dataset).to(self.device) if self.test_data else None

        train_pred = prepare(get_mean_pred(train_batch))
        train_true = {k: v.cpu().numpy() for k, v in train_batch.y.items()}

        test_pred = prepare(get_mean_pred(test_batch)) if self.test_data else None
        test_true = {k: v.cpu().numpy() for k, v in test_batch.y.items()} if self.test_data else None

        valid_batches = [Batch.from_data_list(self.train_valid_data[i][1].dataset).to(self.device) for i in range(len(self.models))]
        ...
        valid_true = {
            target["name"]: np.concatenate([batch.y[target["name"]].cpu().numpy() for batch in valid_batches])
            for target in self.targets
        }
